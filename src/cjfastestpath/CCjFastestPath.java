...
public class CCjFastestPath {
    ...
    public CCjFastestPath(String mapState, int[][] exploredMap) {
        /*
        exploration map = new exploration(mapState);
        exploredMap = map.explore(1);
        */
    ...
    private void movement(List<List<Integer>> stepList) {
        ...
        for (int i = 0; i < stepList.size()-1; i++) {
           ...
           // if(i<stepList.size()-1){
                if (stepList.get(i + 1).get(0) - stepList.get(i).get(0) == 1) instruction += "K"; //right
                else if (stepList.get(i + 1).get(0) - stepList.get(i).get(0) == -1) instruction += "J";
